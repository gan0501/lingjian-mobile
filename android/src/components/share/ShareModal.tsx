...
const ShareModal: FC<ShareModalProps> = ({
  visible, onClose,
  projectName, projectType, constructor, scale, address, publishTime, projectId,
}) => {
  const viewShotRef = useRef<ViewShot>(null);
  const [isGenerating, setIsGenerating] = useState(false);

  const shareUrl = `https://lingjianai.com/project/${projectId}`;
  ...
  const captureAndShare = useCallback(async (action: 'share' | 'wechat' | 'save') => {
    ...
    try {
      ...
      if (action === 'save') {
        ...
        await Share.open({ url: uri, type: 'image/png', title: projectName, message: `【${projectName}】- 来自领建` });
      } else if (action === 'wechat') {
        await Share.open({ url: uri, type: 'image/png', social: Share.Social.WECHAT, title: projectName });
      }
    } catch (e: any) {
      ...
      <View style={styles.overlay}>
        <View style={styles.container}>
          ...
          <View style={styles.header}>
            <Text style={styles.headerTitle}>分享项目</Text>
            ...
            <View style={styles.shareCard}>
              <View style={styles.cardContent}>
                ...
                <View style={styles.tagRow}>
                  <View style={styles.tag}><Text style={styles.tagText}>{projectType}</Text></View>
                </View>
                <View style={styles.infoSection}>
                  <InfoRow label="建设单位" value={constructor} />
                  <InfoRow label="项目规模" value={scale} />
                  <InfoRow label="项目地址" value={address} lines={2} />
...
const styles = StyleSheet.create({
  ...
  projectName: { fontSize: 18, fontWeight: '700', color: '#1a1a1a', lineHeight: 26, marginBottom: 10 },
