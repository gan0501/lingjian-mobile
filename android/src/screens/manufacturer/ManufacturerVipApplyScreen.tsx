...
const CERTIFICATION_FEATURES = [
  { icon: MapPin, text: '厂家LOGO地图展示', desc: '在地图上突出显示您的品牌', color: '#4CAF50' },
  { icon: Globe, text: '厂家详情页专属展示', desc: '定制化厂家主页，提升品牌形象', color: '#2196F3' },
  { icon: ImageIcon, text: '厂家相册展示', desc: '展示厂区环境、生产设备等', color: '#9C27B0' },
  { icon: Award, text: '厂家荣誉展示', desc: '展示资质证书、荣誉奖项', color: '#FF9800' },
  { icon: Package, text: '产品服务展示', desc: '展示核心产品与服务能力', color: '#00BCD4' },
  { icon: Briefcase, text: '项目案例展示', desc: '展示标杆项目，增强客户信任', color: '#E91E63' },
  { icon: Newspaper, text: '厂家新闻发布', desc: '发布厂家动态，提升活跃度', color: '#8BC34A' },
  { icon: Users, text: '销售座席展示', desc: '展示联系人信息，促进商机对接', color: '#FF5722' },
];

type PaymentMethod = 'alipay';
type Props = RootStackScreenProps<'ManufacturerVipApply'>;

const ManufacturerVipApplyScreen: FC<Props> = ({ navigation, route }) => {
  ...
  const handlePayment = useCallback(async () => {
    ...
    try {
      ...
      if (!token) { Alert.alert('提示', '请先登录'); setLoading(false); return; }
      ...
      if (result.success && result.data?.orderInfo) {
        ...
        switch (resultStatus) {
          case '9000':
            Alert.alert('支付成功', '厂家认证已开通，审核将在12小时内完成！', [
              { text: '确定', onPress: () => navigation.goBack() },
            ]);
            break;
          case '6001': Alert.alert('支付取消', '您已取消支付'); break;
          case '6002': Alert.alert('网络错误', '网络连接失败，请检查网络'); break;
          case '4000': Alert.alert('支付失败', memo || '支付失败，请稍后重试'); break;
          default: Alert.alert('支付结果', memo || '未知状态');
        }
      } else {
  ...
  }, [manufacturerId, selectedMethod, loading, navigation]);
  ...
  return (
    <View style={styles.container}>
      ...
      <Header title="认证厂家" showBack onBack={() => navigation.goBack()} transparent />
      <ScrollView
      ...
      >
        <View style={styles.infoCard}>
          ...
          <View style={styles.infoContent}>
            <Text style={styles.infoLabel}>申请认证厂家</Text>
            <Text style={styles.infoValue}>{manufacturerName}</Text>
          </View>
        </View>

        <View style={styles.priceCard}>
          <LinearGradient colors={['rgba(128,1,26,0.3)', 'rgba(255,215,0,0.08)', 'rgba(128,1,26,0.15)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill} />
          <Animated.View style={[styles.shimmerOverlay, { transform: [{ translateX: shimmerTranslate }] }]}>
            <LinearGradient colors={['transparent', 'rgba(255,215,0,0.08)', 'transparent']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.shimmerGradient} />
          </Animated.View>
          <View style={styles.priceHeader}>
            <View style={styles.priceTitleRow}>
              <Crown color={THEME.gold} size={24} />
              <Text style={styles.priceName}>认证厂家</Text>
            </View>
        ...
        <View style={styles.paySection}>
          ...
          <View style={styles.payOption}>
            <Image source={require('@/assets/images/alipay.png')} style={styles.alipayLogo} />
        ...
        <View style={styles.tipsCard}>
          ...
          <Text style={styles.tipsText}>1. 厂家认证仅限已认领的厂家申请</Text>
      ...
      <View style={[styles.bottomBar, { paddingBottom: insets.bottom + 12 }]}>
        ...
        <Animated.View style={[styles.payButtonWrap, { transform: [{ scale: pulseAnim }] }]}>
          <TouchableOpacity style={[styles.payButton, loading && styles.payButtonDisabled]} onPress={handlePayment} disabled={loading} activeOpacity={0.85}>
            <LinearGradient colors={[THEME.primary, '#600012', '#400010']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.payButtonGradient}>
              <Crown color={THEME.gold} size={18} />
              <Text style={styles.payButtonText}>{loading ? '支付中...' : '立即支付 ¥999'}</Text>
            </LinearGradient>
          </TouchableOpacity>
        </Animated.View>
      </View>
...
const styles = StyleSheet.create({
  ...
  infoCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 14, padding: 16, marginBottom: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' },
  infoIconWrap: { width: 40, height: 40, borderRadius: 12, backgroundColor: 'rgba(255,215,0,0.1)', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  infoContent: { flex: 1 },
  infoLabel: { fontSize: 11, color: 'rgba(255,255,255,0.4)', marginBottom: 4 },
  infoValue: { fontSize: 16, color: '#fff', fontWeight: '600' },
  priceCard: { borderRadius: 20, padding: 24, marginBottom: 20, borderWidth: 1, borderColor: 'rgba(255,215,0,0.2)', overflow: 'hidden' },
  shimmerOverlay: { position: 'absolute', top: 0, bottom: 0, width: 80 },
  ...
  officialBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(255,215,0,0.12)', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,215,0,0.2)' },
  ...
  featuresSection: { backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 16, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' },
  featuresSectionHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 16, gap: 8 },
  featuresSectionTitle: { fontSize: 16, color: '#fff', fontWeight: '700', flex: 1 },
  featureCountBadge: { backgroundColor: 'rgba(255,215,0,0.12)', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 10 },
  featureCountText: { fontSize: 11, color: THEME.gold, fontWeight: '600' },
  featureRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.04)' },
  featureIconWrap: { width: 36, height: 36, borderRadius: 10, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  ...
  featureCheck: { width: 24, height: 24, borderRadius: 12, backgroundColor: 'rgba(255,215,0,0.1)', alignItems: 'center', justifyContent: 'center' },
  paySection: { backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 14, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' },
  paySectionTitle: { fontSize: 14, color: 'rgba(255,255,255,0.6)', fontWeight: '600', marginBottom: 12 },
  payOption: { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 10, backgroundColor: 'rgba(128,1,26,0.12)', borderWidth: 1, borderColor: 'rgba(128,1,26,0.3)' },
  ...
  radioActive: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: THEME.primary, alignItems: 'center', justifyContent: 'center' },
  radioInner: { width: 10, height: 10, borderRadius: 5, backgroundColor: THEME.primary },
  tipsCard: { backgroundColor: 'rgba(255,255,255,0.02)', borderRadius: 12, padding: 16 },
  ...
  bottomBar: { position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingTop: 14, backgroundColor: 'rgba(0,0,0,0.92)', borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.06)', gap: 16 },
  priceSummary: {},
  ...
  payButtonGradient: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 14, gap: 8 },
  payButtonDisabled: { opacity: 0.6 },
  payButtonText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  loadingOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', alignItems: 'center', justifyContent: 'center' },
  loadingBox: { backgroundColor: 'rgba(20,20,20,0.95)', borderRadius: 16, paddingHorizontal: 40, paddingVertical: 30, alignItems: 'center', gap: 16, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' },
  loadingText: { color: '#fff', fontSize: 15, fontWeight: '500' },
});

export default ManufacturerVipApplyScreen;
