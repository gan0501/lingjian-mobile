...
import {
  ...
  KeyboardAvoidingView, Platform, TouchableOpacity, Image, ActivityIndicator,
} from 'react-native';
...
import { useEnterpriseDetail } from '@/hooks/useEnterprises';
...
type Props = RootStackScreenProps<'EnterpriseEdit'>;

const EnterpriseEditScreen: FC<Props> = ({ navigation, route }) => {
  const insets = useSafeAreaInsets();
  const { enterpriseId } = route.params;
  const { data: enterprise, isLoading: dataLoading } = useEnterpriseDetail(enterpriseId);
  ...
  useEffect(() => {
    if (enterprise) {
      setEnterpriseName(enterprise.enterprise_name || '');
      setContactPhone(enterprise.contact_phone || '');
      setRegisterAddress(enterprise.register_address || '');
      setDescription(enterprise.description || '');
      setVrUrl(enterprise.vr_url || '');
      setWebsiteUrl(enterprise.url || '');
      setLogoUrl(enterprise.logo_url || '');
      setVideoUrl(enterprise.video_url || '');
      setBrochureUrl(enterprise.brochure_url || '');
      setLicenseUrl(enterprise.license_url || '');
    }
  }, [enterprise]);
  ...
  const handleSave = useCallback(async () => {
    if (!enterpriseName.trim()) {
      Alert.alert('提示', '企业名称不能为空');
      return;
    }

    setSaving(true);
    try {
      const resp = await enterpriseApi.updateEnterprise?.({
  ...
  }, [enterpriseId, enterpriseName, contactPhone, registerAddress, description, vrUrl, websiteUrl, logoUrl, videoUrl, brochureUrl, licenseUrl, navigation]);
  ...
  if (dataLoading) {
    return (
      <View style={styles.container}>
        <Header title="编辑企业信息" showBack onBack={() => navigation.goBack()} />
      ...
      >
        ...
        >
          ...
          <View style={styles.field}>
            <Text style={styles.fieldLabel}>企业名称</Text>
            <TextInput
              style={styles.input}
          ...
          <View style={styles.field}>
            <Text style={styles.fieldLabel}>企业简介</Text>
            <TextInput
          ...
          <View style={styles.field}>
            <View style={styles.fieldLabelRow}>
              ...
              <Text style={styles.fieldLabel}>企业网址</Text>
            </View>
            <TextInput
              style={styles.input}
              value={websiteUrl}
          ...
          <View style={styles.field}>
            ...
            <TextInput
              style={styles.input}
              value={vrUrl}
              onChangeText={setVrUrl}
              placeholder="https://vr.example.com/tour"
              placeholderTextColor="#999"
              autoCapitalize="none"
            />
          </View>

          <Text style={styles.sectionHeader}>企业资料上传</Text>

          {renderUploadItem('企业Logo', 'logo', logoUrl,
            <ImageIcon color="#666" size={16} />)}
          {renderUploadItem('宣传视频', 'video', videoUrl,
            <Video color="#666" size={16} />)}
          {renderUploadItem('产品画册', 'brochure', brochureUrl,
            <FileText color="#666" size={16} />)}
          {renderUploadItem('营业执照', 'license', licenseUrl,
            <FileText color="#666" size={16} />)}
        </ScrollView>
      </KeyboardAvoidingView>
...
export default EnterpriseEditScreen;
