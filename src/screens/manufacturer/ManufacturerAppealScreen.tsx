...
import {
  ...
  Alert, Image, Linking, Platform, KeyboardAvoidingView,
} from 'react-native';
...
type Props = RootStackScreenProps<'ManufacturerAppeal'>;

const ManufacturerAppealScreen: FC<Props> = ({ navigation, route }) => {
  ...
  const handleSubmit = useCallback(async () => {
    if (!licenseUrl) { Alert.alert('提示', '请上传营业执照'); return; }
    if (!idCardUrl) { Alert.alert('提示', '请上传法人身份证'); return; }
    if (!statementUrl) { Alert.alert('提示', '请上传情况说明（需加盖公章）'); return; }
    ...
    try {
      ...
      await enterpriseService.appealManufacturer({
  ...
  }, [manufacturerId, licenseUrl, idCardUrl, statementUrl, navigation]);

  return (
    <View style={styles.container}>
      <Header title="厂家认领申诉" showBack onBack={() => navigation.goBack()} />
      ...
      >
        ...
        >
          ...
          <View style={styles.infoCard}>
            <Text style={styles.infoLabel}>申诉厂家</Text>
            <Text style={styles.infoValue}>{manufacturerName}</Text>
          </View>

          <View style={styles.tipsCard}>
            <Text style={styles.tipsText}>
              如发现该厂家被他人冒领，您可以提交申诉材料。后台将人工审核材料真实性归属进行分辨处理。
            </Text>
          </View>
...
export default ManufacturerAppealScreen;
